...
class Website2021Controller extends Controller
{
    ...
    public function cetakwebsitepdf_2021(){

        $tahun = request('tahun');
        $statuss = \App\Models\Aplikasi::first();
        $instansi_id = \App\Models\User::where('username', session('username'))->first()->instansi_id;
        $nama_instansi = Instansi::where('id', $instansi_id)->first()->nama_instansi;
        $website = \App\Models\Website::Where('instansi_id', $instansi_id)->Where('tahun', '2021')->Where('status', 'Final')->Where('verifikasi', 'Disetujui')->get();
        $hitung_website = \App\Models\Website::Where('instansi_id', $instansi_id)->Where('tahun', '2021')->Where('status', 'Final')->Where('verifikasi', 'Disetujui')->count();
        $penandatanganan = \App\Models\Penandatanganan::Where('instansi_id', $instansi_id)->first();


        $html = View::make('website.2021.cetaklaporanpdf', ['website'=>$website, 'hitung_website'=>$hitung_website, 'penandatanganan'=>$penandatanganan, 'nama_instansi'=>$nama_instansi])->render();
       ...
       /* $pdf->SetCreator(PDF_CREATOR);*/
        ...
        $pdf->Output('Lap. Website 2021.pdf', 'I');
    ...
    public function exportexcel()
    {
        ...
        $html = View::make('website.2021.cetaklaporanexcel', [
            'website' => $website,
