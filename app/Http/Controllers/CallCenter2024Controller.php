...
use App\Exports\LaporanTerkirimCallCenter2024Export;
...
class CallCenter2024Controller extends Controller
{
    public function create(){
        ...
        return view('call_center.2024.create', ['nama_instansi' => $nama_instansi]);
    }
    ...
    public function edit(CallCenter $call_center){
        $instansi_id = \App\Models\User::where('username', session('username'))->first()->instansi_id;
        $nama_instansi = Instansi::where('id', $instansi_id)->first()->nama_instansi;
        return view('call_center.2024.edit', ['call_center' => $call_center, 'nama_instansi' => $nama_instansi]);
    }
    ...
    public function updatestatuscallcenter(Request $request){
        $instansi_id = \App\Models\User::where('username', session('username'))->first()->instansi_id;
        $tahun = request('tahun');
        \App\Models\CallCenter::Where('instansi_id', $instansi_id)->Where('tahun', '2024')->update([
            'status' => 'Final',
        ]);
         return redirect('layanan/index?layanan=aplikasi&jenisaplikasi=call_center&tahun=2024')->with('updatestatus', 'Berhasil Memperbarui Data!');
    }

    public function cetakcallcenterpdf_2024(){

        $tahun = request('tahun');
        $statuss = \App\Models\Aplikasi::first();
        $instansi_id = \App\Models\User::where('username', session('username'))->first()->instansi_id;
        $nama_instansi = Instansi::where('id', $instansi_id)->first()->nama_instansi;
        $call_center = \App\Models\CallCenter::Where('instansi_id', $instansi_id)->Where('tahun', '2024')->Where('status', 'Final')->Where('verifikasi', 'Disetujui')->get();
        $hitung_call_center = \App\Models\CallCenter::Where('instansi_id', $instansi_id)->Where('tahun', '2024')->Where('status', 'Final')->Where('verifikasi', 'Disetujui')->count();
        $penandatanganan = \App\Models\Penandatanganan::Where('instansi_id', $instansi_id)->first();


        $html = View::make('call_center.2024.cetaklaporanpdf', ['call_center'=>$call_center, 'hitung_call_center'=>$hitung_call_center, 'penandatanganan'=>$penandatanganan, 'nama_instansi'=>$nama_instansi])->render();
       ...
       /* $pdf->SetCreator(PDF_CREATOR);*/
        ...
        $pdf->Output('Lap. Layanan Call Center 2024.pdf', 'I');

    }


    public function laporanterkirimcallcenterexcel2024(){
        $instansi_id = \App\Models\User::where('username', session('username'))->first()->instansi_id;
        $nama_instansi = Instansi::where('id', $instansi_id)->first()->nama_instansi;
        $call_center = CallCenter::Where('instansi_id', $instansi_id)->Where('tahun', '2024')->Where('status', 'Final')->get();
        /*return view('aplikasi.2024.laporanterkirimaplikasipublikexcel2024', ['aplikasi' => $aplikasi]);*/
        return Excel::download(new LaporanTerkirimCallCenter2024Export($call_center), 'laporan-call-center-2024.xlsx');
    ...
    public function exportexcel()
    {
        ...
        $html = View::make('call_center.2024.cetaklaporanexcel', [
