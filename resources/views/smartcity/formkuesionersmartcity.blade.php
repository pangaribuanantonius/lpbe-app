...
<div class="card">
	<div class="card-body">
		<form action="{{ route('smartcity.simpan_jawaban') }}" method="post" enctype="multipart/form-data">
			...
			<h2 class="mt-3">Form Kuesioner Smartcity</h2>
			@foreach($kuesioner as $questions)
			<div class="card shadow-lg">
				<div class="card-body">
					...
					<div class="form-group mt-3">
						...
						<input class="ms-3" type="radio" name="jawaban[{{ $questions->id }}]" value="{{ $questions->pilihan1 }}" required> {{ $questions->pilihan1 }} <br>
						<input class="ms-3" type="radio" name="jawaban[{{ $questions->id }}]" value="{{ $questions->pilihan2 }}" required> {{ $questions->pilihan2 }} <br>
						<input class="ms-3" type="radio" name="jawaban[{{ $questions->id }}]" value="{{ $questions->pilihan3 }}" required> {{ $questions->pilihan3 }} <br>
						<input class="ms-3" type="radio" name="jawaban[{{ $questions->id }}]" value="{{ $questions->pilihan4 }}" required> {{ $questions->pilihan4 }} <br>
					</div>
					<input class="ms-3" type="file" name="">
				</div>
			</div>
			@endforeach
			<div>
